...
<html>
	<head>
		...
		<s2:if test="#session.admin==null">
			<title>活跃用户列表</title>
		</s2:if>
		<s2:else>
			<title>系统注册用户列表</title>
		</s2:else>
	</head>
	...
	<DIV class=C_2>
		...
		<DIV class=C_8>
			...
			<DIV class=E_6>
				<s2:set name="hotUser" value="#request.hotUser" />
				<s2:iterator status="offset" value="hotUser" id="user">
<!-- 偶数项 -->
						...
						<DIV class="E_7 r">
							<DIV class=E_8>
								...
								<DIV
									...
									<s2:if test="#session.admin!=null">
									<p><p><a onclick="return(confirm('删除的用户数据将不可恢复，请慎重执行此操作！'))"
										href="deleteUser.action?userId=<s2:property value='id' />"><img
											title="删除该用户" src="images/aa15.gif"
											onmousemove="this.src='images/aa17.gif'"
											onmouseout="this.src='images/aa15.gif'" alt="删除该用户" /></a>
									</s2:if>
						...
						<TD>
							...
							<div>
								<form action="goindex.action" name="pageForm" method="post">
									<dl>
										<dd
											style="text-align: right; font-size: 12px; color: #666633;">
											...
											<s2:if test="#pageInfo.currentPage>0">
												<a href="gethotusers.action" class="nor">首页</a>&nbsp;&nbsp;&nbsp;
									</s2:if>
											<s2:else>
											...
											<s2:if test="#pageInfo.currentPage>0">
												<a
													href="gethotusers.action?currentPage=<s2:property value='#pageInfo.currentPage-1'/>"
													class="nor">上一页</a>&nbsp;
												</s2:if>
											<s2:else>
											...
											<s2:else>
												<a
													href="gethotusers.action?currentPage=<s2:property value='#pageInfo.pageNum-1' />"
													class="nor">末页</a>
											</s2:else>
								</form>
							</div>
